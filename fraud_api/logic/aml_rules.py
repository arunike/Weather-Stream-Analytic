...
class MLAnomalyRule(AMLRule):
    ...
    def evaluate(self, transaction: Dict, context: Dict) -> AMLAlert:
        result = self.detector.score(transaction, context)
        if not result.is_anomaly or result.confidence < self.min_confidence:
            return AMLAlert(
                is_suspicious=False,
                risk_level=AMLRiskLevel.LOW,
                rule_name=self.name,
        ...
        return AMLAlert(
            ...
            reason="ML anomaly detected based on transaction pattern",
            recommended_action="Review transaction and user activity",
            metadata={
                'ml_score': result.score,
                'ml_features': result.metadata
            }
        )


class AMLRuleEngine(RuleEngine[AMLRule, AMLAlert]):
    def __init__(self):
        super().__init__()
        self.rules: List[AMLRule] = []
        # Initialize default rules
        self.add_rule(StructuringRule())
        self.add_rule(RapidMovementRule())
        self.add_rule(HighRiskJurisdictionRule())
        self.add_rule(UnusualPatternRule())
        self.add_rule(MLAnomalyRule())
    ...
    def evaluate(self, payload: Dict, context: Dict) -> List[AMLAlert]:
        alerts = []
    ...
    def evaluate_transaction(
        ...
        return self.evaluate(transaction, context)
