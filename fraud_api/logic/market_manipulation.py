...
class MarketMLAnomalyRule(MarketManipulationRule):
    def __init__(self, min_confidence: float = 0.25, **kwargs):
        super().__init__(priority=6, **kwargs)
        self.min_confidence = min_confidence
        self.model_path = (
            Path(__file__).resolve().parents[2] / 'ml_models' / 'market_anomaly.joblib'
        )
        self.model = self._load_or_train()

    def evaluate(self, trade: Dict, context: Dict) -> Optional[ManipulationAlert]:
        vector = self._extract_features(trade, context)
        ...
        return ManipulationAlert(
            ...
            reason="ML anomaly detected in trade activity",
            evidence=["ML anomaly score exceeded threshold"],
            estimated_impact=trade.get('price', 0) * trade.get('volume', 0),
            recommended_action="Review order flow and participant behavior",
            metadata={
                'ml_score': score,
                'ml_features': {
                    'price': vector[0],
                    'volume': vector[1],
                    'price_change': vector[2],
                    'buy_sell_ratio': vector[3]
                }
            }
        )
    ...
    def _train_default_model() -> IsolationForest:
        ...
        prices = rng.normal(120.0, 40.0, 800)
        volumes = rng.normal(1500.0, 600.0, 800)
        price_change = rng.normal(0.02, 0.05, 800)
        buy_sell_ratio = rng.uniform(0.5, 3.0, 800)
    ...
    @staticmethod
    def _extract_features(trade: Dict, context: Dict) -> List[float]:
        price = float(trade.get('price', 0.0))
        volume = float(trade.get('volume', 0.0))
        price_24h_ago = float(context.get('price_24h_ago', price))
        price_change = (
            (price - price_24h_ago) / price_24h_ago
        ) if price_24h_ago > 0 else 0.0
        buy_sell_ratio = float(context.get('buy_sell_ratio', 1.0))

        return [price, volume, price_change, buy_sell_ratio]

class MarketManipulationEngine(RuleEngine[MarketManipulationRule, ManipulationAlert]):
    def __init__(self):
        super().__init__()
    ...
    def _initialize_default_rules(self):
        ...
        self.add_rule(MarketMLAnomalyRule())
    ...
    def evaluate_trade(
        ...
        return self.evaluate(trade, context)
